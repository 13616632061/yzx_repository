...
import com.ms.adapter.GoodSortListAdapter;
import com.ms.adapter.GoodsListAdapter;
import com.ms.adapter.TodayAllOrderAdapter;
import com.ms.entity.GoodList_Item_check;
import com.ms.entity.GoodSort;
import com.ms.entity.Goods;
import com.ms.entity.Goods_info;
                        ...
                        if (goods_info!=null&&goods_info.length()>0){
                            for(int i=0;i<goods_info.length();i++){
                                ...
                                Goods_info goodsInfo=new Goods_info(jsonObject_goods_info.getString("name"),jsonObject_goods_info.getString("marketable"),
                                        jsonObject_goods_info.getString("goods_id"),JsonUtil.getJsonInt(jsonObject_goods_info,"buy_count"),
                                        JsonUtil.getJsonInt(jsonObject_goods_info,"store"),JsonUtil.getJsonDouble(jsonObject_goods_info,"price"),
                                        jsonObject_goods_info.getString("img_src"), JsonUtil.getJsonInt(jsonObject_goods_info,"btn_switch_type"),0,"",false,0,"","");
        ...
        switch (parent.getId()){
            case R.id.list_sort:
               ...
               if(goodsListAdapter.getCheckid().replace(",","").length()>0){
                   ...
                   tv_message.setText("有选择的商品未下架，      是否离开？");
                   btn_sure.setText("确定");
                   btn_dialog_cell.setText("取消");
