...
import com.ms.ks.NearlyShopperGoodsActivity;
import com.ms.ks.R;
import com.ms.ks.ShoppingSortNearlyShopperActivity;
...
public class AllSupplyOrderFragmentAdapter extends BaseAdapter {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        mHorlder.tv_money.setText("￥"+SetEditTextInput.stringpointtwo(mSupplyOrderPageOrderlist.get(position).getFinal_amount())+"（含配送费￥"+SetEditTextInput.stringpointtwo(mSupplyOrderPageOrderlist.get(position).getCost_freight())+")");
        mHorlder.tv_nums.setText("共"+mSupplyOrderPageOrderlist.get(position).getTotal_quantity()+"件商品");
        if(mSupplyOrderPageOrderlist.get(position).getPay_status().equals("0")){
            mHorlder.tv_paystatus.setText("待付款");
        }
        if(mSupplyOrderPageOrderlist.get(position).getPay_status().equals("1")&&mSupplyOrderPageOrderlist.get(position).getShip_status().equals("0")){
            mHorlder.tv_paystatus.setText("待发货");
        }
        if(mSupplyOrderPageOrderlist.get(position).getPay_status().equals("1")&&mSupplyOrderPageOrderlist.get(position).getShip_status().equals("1")){
            mHorlder.tv_paystatus.setText("待收货");
        }
        if(mSupplyOrderPageOrderlist.get(position).getStatus().equals("finish")){
            mHorlder.tv_paystatus.setText("交易成功");
        }
        return convertView;
    }
